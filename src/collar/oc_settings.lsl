...
This file has been adapted to zCollar for non-database settings
Copyright ©2021
...
default
{
    on_rez(integer t){
        ...
        if(llGetInventoryType("zni_settings")==INVENTORY_SCRIPT)llRemoveInventory("zni_settings"); // There can be only one
        g_iCurrentIndex=0;
        llSetTimerEvent(10);
    }
    state_entry()
            ...
            if(iMenuIndex!=-1){
                ...
                if(sMenu == "Reboot"){
                    if(sMsg=="No")return;
                        ...
                        llMessageLinked(LINK_SET, 0, "reboot", kAv);
                    }
