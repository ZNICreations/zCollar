...
Menu(key kID, integer iAuth) {
    ...
    DialogAddon(kID, sPrompt, lButtons, ["DISCONNECT", UPMENU], 0, iAuth, "Menu~Main");
}
...
default
{
    ...
    listen(integer channel, string name, key id, string msg){
        ...
        if (sPacketType == "approved" && g_kCollar == NULL_KEY)
        {
            ...
            g_kCollar = NULL_KEY;
            llSay(0, "Connection request was denied by the collar");
            llResetScript();
        }
                ...
                if (iNum == LM_SETTING_RESPONSE)
                {
                    ...
                    if(sStr == "settings=sent"){
                        if(g_iAddonLimitation){
                            llSay(0, "Error: Addon limitations are in place. Unweld tool cannot continue. To disable the addon limitation, please see the settings menu of your collar, inside the Addon.. submenu you will find the Limiter which must be disabled.");
                        ...
                        if (sMenu == "Menu~Main")
                        {
                            ...
                            else if (sMsg == "UNWELD NOW")
                            {
                                if(iAuth & (C_OWNER|C_WEARER)){
                                    ...
                                    llSay(0, "Weld is now broken");
                                    llResetScript();
                                }else{
                                    llSay(0, "This tool cannot be used by someone with public access, your collar access level must be owner or wearer");
                                }
                            }
